...
sudo apt install nload && sudo apt install mc -y && sudo apt install docker.io -y && sudo apt install nload && sudo apt install cbm -y && sudo apt install ethtool -y && sudo apt install net-tools
wget http://archive.ubuntu.com/ubuntu/pool/universe/m/miniupnpd/miniupnpd_2.1-6.1_amd64.deb
...
sudo dpkg -i miniupnpd_2.1-6.1_amd64.deb
sudo apt install -f
...
privateIp=$(ip addr show $net | grep "inet " | grep -v 127.0.0.1 | awk 'match($0, /(10\.[0-9]+\.[0-9]+\.[0-9]+)/) {print substr($0,RSTART,RLENGTH)}' | head -n1)
if [[ -z "$privateIp" ]]; then
    privateIp=$(ip addr show $net | grep "inet " | grep -v 127.0.0.1|awk 'match($0, /([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)/) {print substr($0,RSTART,RLENGTH)}' | head -n1)
fi
